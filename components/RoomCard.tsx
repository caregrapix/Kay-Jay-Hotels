...
// Helper map and function for categorizing features
const featureCategoryMap: { [key: string]: string } = {
  // View & Location
  'sea view': 'View & Location',
  'garden view': 'View & Location',
  'plantation view': 'View & Location',
  'jungle view': 'View & Location',
  'superior view': 'View & Location',
  'panoramic views': 'View & Location',
  'pool view': 'View & Location',
  'prime location': 'View & Location',
  'private balcony': 'View & Location',
  'private veranda': 'View & Location',

  // Bed & Bath
  'hot water': 'Bed & Bath',
  'attached bathroom': 'Bed & Bath',
  'spacious bathroom': 'Bed & Bath',
  'modern bathroom': 'Bed & Bath',
  'plush bedding': 'Bed & Bath',
  'comfortable bedding': 'Bed & Bath',
  'extra bed': 'Bed & Bath',
  'extra bedding': 'Bed & Bath',
  'bunk beds': 'Bed & Bath',

  // Amenities & Comfort
  'air conditioning': 'Amenities & Comfort',
  'premium amenities': 'Amenities & Comfort',
  'enhanced amenities': 'Amenities & Comfort',
  'essential amenities': 'Amenities & Comfort',
  'mini fridge': 'Amenities & Comfort',
  'television': 'Amenities & Comfort',
  'cozy & comfortable': 'Amenities & Comfort',
  'comfortable & cozy': 'Amenities & Comfort',
  'premium comfort': 'Amenities & Comfort',
  
  // Layout & Space
  'spacious layout': 'Layout & Space',
  'extra space': 'Layout & Space',
  'separate living area': 'Layout & Space',
  'spacious for three': 'Layout & Space',
  
  // Group Features
  'shared facilities': 'Group Features',
  'ideal for groups': 'Group Features',
  'perfect for large groups': 'Group Features',
  'ideal for solo travelers': 'Group Features'
};

const getCategorizedFeatures = (features: string[]): { [category: string]: string[] } => {
  const grouped: { [category: string]: string[] } = {};
  
  features.forEach(feature => {
    const category = featureCategoryMap[feature.toLowerCase()] || 'Highlights';
    if (!grouped[category]) {
      grouped[category] = [];
    }
  ...
  const categoryOrder = ['View & Location', 'Layout & Space', 'Bed & Bath', 'Amenities & Comfort', 'Group Features', 'Highlights'];
  const sortedGrouped: { [category: string]: string[] } = {};
  
  // Add categories to the sorted object in the desired order
  ...
  // Add any remaining categories that were not in the order list (just in case)
...
const RoomCard: React.FC<RoomCardProps> = ({ 
    ...
    const [paymentMethod, setPaymentMethod] = React.useState('standard'); // standard, commercial, hnb
    const [error, setError] = React.useState<string | null>(null);
    const [isDisclaimerAccepted, setIsDisclaimerAccepted] = React.useState(false);
    
    const isCustomInquiry = React.useMemo(() => {
        const totalGuests = adults + children;
        if (room.totalCapacity) {
            return totalGuests > room.totalCapacity;
        }
        return adults > room.maxAdults || children > room.maxChildren;
    }, [adults, children, room]);

    const basePriceLKR = room.price[mealPlan];
    const convertedBasePrice = convertPrice(basePriceLKR);
    const isDiscounted = paymentMethod === 'commercial' || paymentMethod === 'hnb';
    const discountedPrice = isDiscounted && convertedBasePrice ? convertedBasePrice * 0.85 : convertedBasePrice;
    ...
    const finalPriceString = formatPrice(discountedPrice);

    const categorizedFeatures = getCategorizedFeatures(room.features);
    ...
    const handleInquiry = () => {
        ...
        const params = new URLSearchParams({
            ...
            custom: isCustomInquiry.toString(),
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200/80 flex flex-col lg:flex-row">
        <div className="lg:w-2/5">
          <LazyImage src={room.image} alt={room.name} className="w-full h-64 lg:h-full object-cover" />
        </div>
        <div className="lg:w-3/5 p-6 flex flex-col justify-between">
          <div className="flex-grow">
            <h3 className="text-2xl font-bold font-sans text-brand-dark">{room.name}</h3>
          ...
          <div className="mt-6 bg-brand-gray/50 p-4 rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select id={`meal-plan-${room.id}`} value={mealPlan} onChange={e => setMealPlan(e.target.value as 'ro' | 'bb' | 'hf' | 'fb')} className={inputClasses}>
                  ...
                  <option value="hf">Half Board (HB)</option>
                  <option value="fb">Full Board (FB)</option>
                </select>
              </div>
              <div className="md:col-span-2">
                 <label htmlFor={`payment-method-${room.id}`} className={labelClasses}>Payment Method</label>
                 <select id={`payment-method-${room.id}`} value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} className={inputClasses}>
                    <option value="standard">Standard Rate / Pay at Hotel</option>
                    <option value="commercial">Commercial Bank Card (15% Off)</option>
                    <option value="hnb">HNB Credit Card (15% Off)</option>
                 </select>
              </div>
               ...
               <div className="md:col-span-2 text-right">
                ...
                {isDiscounted ? (
                    <div className="flex items-baseline justify-end gap-2">
                        <del className="text-gray-500 text-lg">{formatPrice(convertedBasePrice)}</del>
                        <span className="font-bold text-brand-primary text-2xl">{finalPriceString}</span>
                    </div>
                ) : (
                    <p className="font-bold text-brand-dark text-2xl">{finalPriceString}</p>
                )}
              </div>
              ...
              <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button 
                    ...
                    className={`w-full bg-brand-primary text-white font-bold py-3 px-4 rounded-md hover:bg-brand-dark transition-colors duration-300 flex items-center justify-center ${disabledButtonClasses}`}
                >
                ...
                <WhatsAppButton 
                  onClick={handleInquiry}
                  className={`w-full bg-green-500 text-white font-bold py-3 px-4 rounded-md hover:bg-green-600 transition-colors duration-300 flex items-center justify-center ${disabledButtonClasses}`}
                  disabled={!isDisclaimerAccepted}
                >
                  WhatsApp Inquiry
                </WhatsAppButton>
            ...
            {isCustomInquiry && (
              <p className="text-yellow-700 text-sm mt-3 bg-yellow-100 p-2 rounded-md">
