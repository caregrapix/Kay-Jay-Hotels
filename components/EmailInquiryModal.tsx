...
const EmailInquiryModal: React.FC<EmailInquiryModalProps> = ({
  ...
  return (
    ...
    >
      <div 
        className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto transform transition-transform duration-300 scale-100"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-8 md:p-10">
            <div className="flex justify-between items-start mb-6">
                 <h1 id="inquiry-modal-title" className="text-3xl font-sans font-bold text-brand-dark">Email Inquiry</h1>
            ...
            <div className="bg-brand-gray/50 p-6 rounded-lg mb-8 border border-gray-200">
                <h2 className="text-2xl font-bold font-sans text-brand-dark mb-4">Inquiry Summary</h2>
                {isCustom && (
            ...
            {formStatus === 'success' ? (
                <div className="text-center p-6 bg-green-50 border-l-4 border-green-500 rounded-md">
                    ...
                    <button onClick={onClose} className="mt-4 bg-brand-primary text-white font-bold py-2 px-6 rounded-md hover:bg-brand-dark transition-all duration-300">
                        Close
                    </button>
                </div>
            ) : (
                <form onSubmit={handleSubmit} noValidate>
                    <h2 className="text-2xl font-bold font-sans text-brand-dark mb-4">Your Contact Information</h2>
                    <div className="space-y-6">
                        <div>
                            <label htmlFor="modal-name" className="block text-sm font-medium text-gray-700 mb-1">Full Name</label>
                            <input type="text" id="modal-name" name="name" value={formData.name} onChange={handleChange} className={`w-full text-base px-4 py-3 border rounded-md focus:ring-brand-primary focus:border-brand-primary ${errors.name ? 'border-red-500' : 'border-gray-300'}`} required aria-required="true" />
                            {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
                        </div>
                        <div>
                            <label htmlFor="modal-email" className="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
                            <input type="email" id="modal-email" name="email" value={formData.email} onChange={handleChange} className={`w-full text-base px-4 py-3 border rounded-md focus:ring-brand-primary focus:border-brand-primary ${errors.email ? 'border-red-500' : 'border-gray-300'}`} required aria-required="true" />
                        ...
                        <div>
                            ...
                            <input type="tel" id="modal-phone" name="phone" value={formData.phone} onChange={handleChange} className="w-full text-base px-4 py-3 border border-gray-300 rounded-md focus:ring-brand-primary focus:border-brand-primary" />
                        </div>
                    </div>
                    <div className="mt-8 flex flex-col sm:flex-row-reverse items-center gap-4">
                        <button type="submit" disabled={formStatus === 'submitting'} className="w-full sm:w-auto bg-brand-primary text-white font-bold py-3 px-6 rounded-md hover:bg-brand-dark transition-all duration-300 disabled:bg-gray-400 disabled:cursor-not-allowed">
                            {formStatus === 'submitting' ? 'Sending...' : 'Send Inquiry'}
                        </button>
                        <button type="button" onClick={onClose} className="w-full sm:w-auto bg-gray-200 text-gray-700 font-bold py-3 px-6 rounded-md hover:bg-gray-300 transition-all duration-300">
                            Cancel
                        </button>
                    </div>
