...
const BlogCard: React.FC<BlogCardProps> = ({ post }) => {
  ...
  return (
    <Link to={`/blog/${post.slug}`} className="group block bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 ease-in-out border border-transparent hover:border-brand-primary/50 hover:shadow-xl h-full flex flex-col">
      <div className="relative h-56 overflow-hidden">
        {featuredImage ? (
          ...
          <div className="w-full h-full bg-brand-gray flex items-center justify-center">
            <img src="/public/images/logo.png" alt="Kayjay Hotels Logo" className="h-10 opacity-50" />
          </div>
        )}
      </div>
