    ...
    <section className="bg-brand-gray/80 py-8">
      <div className="container mx-auto px-6">
        <div className="bg-white p-6 rounded-lg shadow-lg">
          <h2 className="text-2xl font-sans font-bold text-brand-dark mb-4">Check Availability at {hotel.name}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <div>
              <label htmlFor="top-check-in" className="block text-sm font-medium text-gray-700">Check-in</label>
              <input type="date" id="top-check-in" name="checkIn" value={checkIn} onChange={handleChange} min={today} className="mt-1 w-full text-base px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:ring-brand-primary focus:border-brand-primary" />
            </div>
            <div>
              <label htmlFor="top-check-out" className="block text-sm font-medium text-gray-700">Check-out</label>
              <input type="date" id="top-check-out" name="checkOut" value={checkOut} onChange={handleChange} min={checkIn || today} className="mt-1 w-full text-base px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:ring-brand-primary focus:border-brand-primary" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="top-adults" className="block text-sm font-medium text-gray-700">Adults</label>
                <select id="top-adults" name="adults" value={adults} onChange={handleChange} className="mt-1 block w-full text-base px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:ring-brand-primary focus:border-brand-primary">
                  {generateOptions(1, 10)}
                </select>
              </div>
              <div>
                <label htmlFor="top-children" className="block text-sm font-medium text-gray-700">Children</label>
                <select id="top-children" name="children" value={children} onChange={handleChange} className="mt-1 block w-full text-base px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:ring-brand-primary focus:border-brand-primary">
