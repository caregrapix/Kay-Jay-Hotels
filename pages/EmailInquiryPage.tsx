...
const EmailInquiryPage: React.FC = () => {
  ...
  const inquiryData = React.useMemo(() => {
    return {
      ...
      paymentMethod: searchParams.get('paymentMethod') || 'standard',
  ...
  const getPaymentMethodLabel = (method: string) => {
    switch(method) {
        case 'commercial': return 'Commercial Bank Card (15% Off)';
        case 'hnb': return 'HNB Credit Card (15% Off)';
        default: return 'Standard Rate / Pay at Hotel';
    }
  };
...
*Guests:* ${inquiryData.adults} Adults, ${inquiryData.children} Children`;
    ...
    if (inquiryData.paymentMethod !== 'standard') {
        const cardName = inquiryData.paymentMethod === 'commercial' ? 'Commercial Bank' : 'HNB';
        message += `\n*Payment Method:* ${cardName} Credit Card (15% Off)`;
        message += `\n*Quoted Price:* ${inquiryData.finalPrice}`;
    } else {
...
Thank you.`;
  ...
  return (
    <>
      <PageMetadata
        title="Booking Inquiry | Luxury in Every Detail | Kayjay Hotels"
        description="Finalize your booking inquiry. Confirm your details and send the request to our reservations team."
      />
      <div className="bg-brand-gray py-20">
        <div className="container mx-auto px-6">
          <AnimatedSection>
            <div className="max-w-3xl mx-auto bg-white p-8 md:p-10 rounded-lg shadow-xl">
              <h1 className="text-3xl md:text-4xl font-sans font-bold text-brand-dark mb-2">Confirm Your Inquiry</h1>
              ...
              <div className="bg-brand-gray/50 p-6 rounded-lg mb-8 border border-gray-200">
                ...
                <div className="space-y-3 text-gray-800">
                  ...
                  <p><strong>Payment Method:</strong> {getPaymentMethodLabel(inquiryData.paymentMethod)}</p>
                  <p><strong>Quoted Price:</strong> <span className="font-bold">{inquiryData.finalPrice}</span></p>
                </div>
              </div>
              ...
              {formStatus === 'success' ? (
                ...
                <form onSubmit={handleEmailSubmit} noValidate>
                  ...
                  <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button type="submit" disabled={formStatus === 'submitting' || !isDisclaimerAccepted} className={`w-full bg-brand-primary text-white font-bold py-3 px-6 rounded-md hover:bg-brand-dark transition-all duration-300 flex items-center justify-center ${disabledButtonClasses}`}>
                     ...
                     <WhatsAppButton
                        ...
                        className={`w-full bg-green-500 text-white font-bold py-3 px-6 rounded-md hover:bg-green-600 transition-colors duration-300 flex items-center justify-center ${disabledButtonClasses}`}
                    >
                        Send via WhatsApp
                    </WhatsAppButton>
                  </div>
